...
import com.xinyan.spider.isp.mobile.parser.cmcc.BeiJingCmccParser;
 ...
 */
@Component
public class BeiJingCmccPssor extends AbstractProcessor {

    @Autowired
    private BeiJingCmccParser parser;

    protected static Logger logger = LoggerFactory.getLogger(BeiJingCmccPssor.class);
    ...
    public Result doLogin(WebClient webClient, Context context) {
        ...
        try {
            ...
            Page page = getPage(webClient, url, HttpMethod.GET, null,3,null, null);
    ...
    public Result processBillInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        try {
            ...
            String logFlag = String.format("==>6.采集账单信息," +
                    "第[{}]次尝试请求.....");
            Page resultPage = getPage(webClient, url, HttpMethod.GET, reqParam, Constants.MAX_SENDMSG_TIIMES,
                    logFlag, null);
            if (null != resultPage) {
