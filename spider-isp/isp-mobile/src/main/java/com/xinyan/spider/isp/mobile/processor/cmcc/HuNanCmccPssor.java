...
import com.xinyan.spider.isp.mobile.parser.cmcc.HuNanCmccParser;
import com.xinyan.spider.isp.mobile.parser.cmcc.ShanDongCmccParser;
 ...
 */
@Component
public class HuNanCmccPssor extends AbstractProcessor {

    @Autowired
    private HuNanCmccParser parser;

    protected static Logger logger = LoggerFactory.getLogger(HuNanCmccPssor.class);
    ...
    public Result doLogin(WebClient webClient, Context context) {
        ...
        try {
            ...
            if ("{\"needVerifyCode\":\"1\"}".equals(page.getWebResponse().getContentAsString())) {
                ...
                } else if("2".equals(page.getWebResponse().getContentAsString())){
                    logger.info("==>[{}]短信下发数已达上限，您可以使用服务密码方式登录！", context.getTaskId());
                    result.setResult(StatusCode.短信验证码发送次数超过上限);
                    return result;
                }else{
                ...
                url = "https://login.10086.cn/login.htm?accountType=01&account=" + context.getUserName() + "&password="
                        + context.getPassword() + "&pwdType=01&smsPwd=&inputCode=&backUrl=http%3A%2F%2Fshop.10086" +
                        ".cn%2Fi%2F&rememberMe=0&channelID=12003&protocol=https%3A&timestamp=1471507724633";
                page = getPage(webClient, url, HttpMethod.GET, null,Constants.MAX_SENDMSG_TIIMES,null, header);
    ...
    protected Result doLoginBySMS(WebClient webClient, Context context) throws Exception {
        ...
        try {
            ...
            String url = "https://login.10086.cn/login.htm?accountType=01&account=" + context.getUserName() +
                    ...
                    "&inputCode=&backUrl=http://shop.10086.cn/mall_250_250.html?forcelogin=1&rememberMe=0&channelID=12002&protocol=https:&timestamp=1471498940062";
...
//            }
    ...
    public Result processBaseInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        try {
            ...
            logger.info("==>[{}]1.3抓取city信息开始.....", context.getTaskId());
            url = String.format("http://shop.10086.cn/i/v1/res/numarea/%s?_=1504784153383 ", context.getUserName());
            resultPage = getPage(webClient, url, HttpMethod.GET, null,Constants.MAX_SENDMSG_TIIMES,null, header);
            //存入缓存
            if (null != resultPage) {
            ...
            logger.info("==>[{}]1.3抓取city信息结束.", context.getTaskId());
            result.setSuccess();
    ...
    protected Result processUserRechargeItemInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        try {
            ...
            String url = "http://shop.10086.cn/i/v1/cust/his/"+context.getUserName()+"?startTime="+ DateFormatUtils.format(calendar, "yyyyMMdd")+"&endTime="+DateFormatUtils.format(Calendar.getInstance(), "yyyyMMdd")+"&_="+calendar.getTimeInMillis();
    ...
    private Result GetContentInfo(WebClient webClient, CacheContainer cacheContainer, Context context, String
        ...
        try {
            for (int i = 0; i >= -5; i--) {
                ...
                } else if(StringUtils.contains(resultPage.getWebResponse().getContentAsString(), "您选择时间段没有详单记录")){
                    logger.info("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + _month + "]" + strType + "-->.您选择时间段没有详单记录");
                    continue;
