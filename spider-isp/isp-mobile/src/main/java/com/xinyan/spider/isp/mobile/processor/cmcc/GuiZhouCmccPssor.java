...
import com.xinyan.spider.isp.mobile.parser.cmcc.GuiZhouCmccParser;
 ...
 */
@Component
public class GuiZhouCmccPssor extends AbstractProcessor {

    @Autowired
    private GuiZhouCmccParser parser;

    protected static Logger logger = LoggerFactory.getLogger(GuiZhouCmccPssor.class);
    ...
    public Result doLogin(WebClient webClient, Context context) {
        ...
        try {
            ...
            url = "https://login.10086.cn/html/login/login.html?channelID=12003";
...
//            }
    ...
    public Result processBillInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        List<NameValuePair> reqParam = new ArrayList<>();
        try {
            String amonth = DateUtils.getDiffMonth("yyyyMM", -5);
            url = String.format("http://shop.10086.cn/i/v1/fee/billinfo/%s?bgnMonth=%s&endMonth=%s&_=1496972564998", context.getUserName(), amonth, DateUtils.getCustomYearMont("yyyyMM"));
    ...
    private Result GetContentInfo(WebClient webClient, CacheContainer cacheContainer, Context context, String
        ...
        try {
            for (int i = 0; i >= -5; i--) {
                ...
                String logFlag = String.format("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]第1页" + strType + "信息,第[{}]次尝试请求.....");
                ...
                url = String.format("https://shop.10086.cn/i/v1/fee/detailbillinfojsonp/%s?callback=jQuery18308911999785481797_1494312492711&curCuror=1&step=100&qryMonth=%s&billType=%s&_=1433989116212",
                        context.getUserName(), month, strCode);
                ...
                if (StringUtils.contains(resultPage.getWebResponse().getContentAsString(), "retCode\":\"000000")) {
                    ...
                    logger.info("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]" + strType + "成功.");
                    ...
                    logger.info("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]" + strType + "失败.");
                    continue;
                }
                ...
                for (int j = 1; j < iPageTotal / 100 + 1; j++) {
                    url = String.format("https://shop.10086.cn/i/v1/fee/detailbillinfojsonp/%s?callback=jQuery18308911999785481797_1494312492711&curCuror=%s&step=100&qryMonth=%s&billType=%s&_=1433989116212"
                            , context.getUserName(), j + 100 * j, month, strCode);
                    logFlag = String.format("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]第" + (j + 1) + "页" + strType + "信息,第[{}]次尝试请求.....");
                    resultPage = getPage(webClient, url, HttpMethod.GET, null, Constants.MAX_RETRY_TIIMES, logFlag, header);
                    if (StringUtils.contains(resultPage.getWebResponse().getContentAsString(), "retCode\":\"000000")) {
                        pageList.add(resultPage);
                        logger.info("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]第" + (j + 1) + "页" + strType + "成功.");
                    } else {
                        logger.info("==>" + strIndex + "." + (Math.abs(i) + 1) + "采集[" + month + "]第" + (j + 1) + "页" + strType + "失败.");
                    }
                }
            }
