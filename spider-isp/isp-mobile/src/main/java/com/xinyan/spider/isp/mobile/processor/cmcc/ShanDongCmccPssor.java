 ...
 */
@Component
public class ShanDongCmccPssor extends AbstractProcessor {

    @Autowired
    private ShanDongCmccParser parser;

    protected static Logger logger = LoggerFactory.getLogger(ShanDongCmccPssor.class);
    ...
    public Result doLogin(WebClient webClient, Context context) {
        ...
        try {
            ...
            if ("{\"needVerifyCode\":\"1\"}".equals(page.getWebResponse().getContentAsString())) {
                ...
                header.clear();
                header.put("Referer", "https://login.10086.cn/login.html?channelID=12003");
...
//            }
    ...
    public Result processBaseInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        try {
            ...
            url = String.format("http://shop.10086.cn/i/v1/res/numarea/%s?_=1496714847438", context.getUserName());
            resultPage = getPage(webClient, url, HttpMethod.GET, null,Constants.MAX_SENDMSG_TIIMES,null, header);
            //存入缓存
            if (null != resultPage) {
