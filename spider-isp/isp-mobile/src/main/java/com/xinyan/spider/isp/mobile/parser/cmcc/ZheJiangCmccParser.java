 ...
 * @version V1.0
 * @description
 * @date 2017年5月16日 下午1:41:05
 */
@Component
public class ZheJiangCmccParser {
	protected static Logger logger = LoggerFactory.getLogger(ZheJiangCmccParser.class);
	...
	public Result basicInfoParse(Context context, CacheContainer cacheContainer, CarrierInfo carrierInfo) {
		...
		//carrierInfo.getCarrierUserInfo().setName(context.getIdName());
...
//		}
		carrierInfo.getCarrierUserInfo().setProvince("浙江"); // 所属省份
				...
				if (smsDataArr != null) {
					for (int i = 0; i < smsDataArr.size(); i++) {
						...
						carrierSmsRecordInfo.setBillMonth(DateUtils.dateToString(
								DateUtils.stringToDate(jsonObject.getString("startDate"), "yyyyMMdd"), "yyyy-MM")); // 通话月份，格式：yyyy-MM
						carrierSmsRecordInfo.setTime(
								carrierSmsRecordInfo.getBillMonth().substring(0, 5) + smsData.getString("startTime")); // 发送时间
						...
						if ("发".equals(smsData.getString("commMode"))) {
							carrierSmsRecordInfo.setSendType("SEND");// SEND-发送;   // RECEIVE-收取
						} else if ("收".equals(smsData.getString("commMode"))) {
						...
						carrierSmsRecordInfo.setServiceName(smsData.getString("busiName"));// 业务名称.
						// e.g.
						// 点对点(网内)
			...
			for (Page page : pages) {
				...
				if (netDataArr != null) {
					for (int i = 0; i < netDataArr.size(); i++) {
						...
						String[] sumFlowArr = netData.getString("sumFlow").replace("GB", ":").replace("MB", ":")
								.replace("KB", ":").replaceAll(" ", "").split(":");
						int sumFlow = 0;
						if (sumFlowArr.length == 3) {
							sumFlow = Integer.parseInt(sumFlowArr[0]) * 1024 * 1024 + Integer.parseInt(sumFlowArr[1]) * 1024
									+ Integer.parseInt(sumFlowArr[2]);
						} else if (sumFlowArr.length == 2) {
							sumFlow = Integer.parseInt(sumFlowArr[0]) * 1024 + Integer.parseInt(sumFlowArr[1]);
						} else {
							sumFlow = Math.round(Float.parseFloat(sumFlowArr[0]));
						}
		...
		if(page != null){
			...
			if(dataArr != null) {
				...
				for (Object object : arrArr) {
					...
					JSONArray secResInfoArr = resInfos.getJSONArray("secResInfos");
