 ...
 */
@Component
public class GuiZhouCmccParser {
	protected static Logger logger = LoggerFactory.getLogger(GuiZhouCmccParser.class);
...
//		}
		carrierInfo.getCarrierUserInfo().setProvince("贵州"); // 所属省份
			...
			for (Page page : pages) {
				...
				if (callDataArr != null) {
					for (int i = 0; i < callDataArr.size(); i++) {
						...
						// e.g.省内漫游
						if (callData.getString("commMode").contains("主叫")) {
							carrierCallDetailInfo.setDialType("DIAL");
						} else if (callData.getString("commMode").contains("被叫")) {
			...
			for (Page page : pages) {
				...
				if (smsDataArr != null) {
					for (int i = 0; i < smsDataArr.size(); i++) {
						...
						if (smsData.getString("commMode").contains("发送")) {
							carrierSmsRecordInfo.setSendType("SEND");// SEND-发送;   // RECEIVE-收取
						} else if (smsData.getString("commMode").contains("接收")) {
