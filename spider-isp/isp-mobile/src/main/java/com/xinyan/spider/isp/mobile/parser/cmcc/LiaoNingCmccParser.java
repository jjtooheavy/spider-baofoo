 ...
 */
@Component
public class LiaoNingCmccParser {
	protected static Logger logger = LoggerFactory.getLogger(LiaoNingCmccParser.class);
...
//		}
			carrierInfo.getCarrierUserInfo().setProvince("辽宁"); // 所属省份
		...
		if (pages != null) {
			for (Page page : pages) {
				...
				for (int i = 0; i < smsDataArr.size(); i++) {
					...
					}else if("收取".equals(smsData.getString("commMode"))){
