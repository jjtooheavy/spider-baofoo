 ...
 */
@Component
public class GanSuCmccParser {
	protected static Logger logger = LoggerFactory.getLogger(GanSuCmccParser.class);
...
//		}
		carrierInfo.getCarrierUserInfo().setProvince("甘肃"); // 所属省份
				...
				if (netDataArr != null) {
					for (int i = 0; i < netDataArr.size(); i++) {
						...
						if (StringUtils.isNotEmpty(netData.getString("commTime"))) {
							String[] durationArr = netData.getString("commTime").replace("小时",":").replace("分",":").replace("秒","").split(":");
							int duration = 0;
						...
						int sumFlow = Integer.parseInt(netData.getString("sumFlow").replace("KB",""));
