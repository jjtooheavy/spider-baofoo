...
import com.xinyan.spider.isp.mobile.parser.cmcc.GanSuCmccParser;
 ...
 */
@Component
public class GanSuCmccPssor extends AbstractProcessor {

    @Autowired
    private GanSuCmccParser parser;

    protected static Logger logger = LoggerFactory.getLogger(GanSuCmccPssor.class);
        ...
        try {
            ...
            if ("{\"needVerifyCode\":\"1\"}".equals(page.getWebResponse().getContentAsString())) {
                ...
                if (!"true".equals(page.getWebResponse().getContentAsString())) {
                    ...
                    return new Result(StatusCode.验证手机号码失败);
                }
    ...
    public Result processBillInfo(WebClient webClient, Context context, CarrierInfo carrierInfo, CacheContainer cacheContainer) {
        ...
        try {
            ...
            if (null != resultPage) {
                ...
                return new Result(StatusCode.解析账单信息出错);
            }
            result.setSuccess();
        } catch (Exception e) {
    ...
    private Result GetContentInfo(WebClient webClient, CacheContainer cacheContainer, Context context, String
        ...
        try {
            for (int i = 0; i >= -5; i--) {
                ...
                url = String.format("https://shop.10086.cn/i/v1/fee/detailbillinfojsonp/%s?callback=jQuery18308911999785481797_1494312492711&curCuror=1&step=100&qryMonth=%s&billType=%s&_="+System.currentTimeMillis(),
            ...
            if (page.getWebResponse().getContentAsString().contains("\"retMsg\":\"success\"")) {
                ...
                if (isFlag) {
                    ...
                    if("SMS_CODE".equals(verify_code)){
                        header.put("Referer", "http://shop.10086.cn/i/?f=home&welcome="+context.getParam1());
                        url = "https://shop.10086.cn/i/v1/fee/detbillrandomcodejsonp/" + context.getUserName() + "?callback=jQuery18305713919829137077_1504058488141&_=" + Calendar.getInstance().getTimeInMillis();
                        page = getPage(webClient, url, HttpMethod.GET, null,Constants.MAX_SENDMSG_TIIMES,null, header);
